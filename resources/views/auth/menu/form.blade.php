<form class="form-horizontal"
    action="{{ $model->exists ? route('auth.menu.update', base64_encode($model->id)) : route('auth.menu.store') }}"
    method="POST">
    {{ csrf_field() }}
    @if ($model->exists)
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" name="name" id="name" class="form-control" readonly placeholder="Name"
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" name="display_name" id="display_name" class="form-control" placeholder="Display Name"
                @if ($model->exists) value="{{ $model->display_name }}" @endif>
        </div>
    </div>
        ...
        <div class="col-sm-10">
            <select name="menu_id" id="menu_id" class="select2" style="width: 100%;">
                ...
                @foreach (App\Models\Auth\Menu::all() as $menu)
                    <option {{ $model->menu_id == $menu->id ? 'selected' : '' }} value="{{ $menu->id }}">
                        {{ $menu->display_name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" name="url" id="url" class="form-control" placeholder="url"
                @if ($model->exists) value="{{ $model->url }}" @endif>
        </div>
    </div>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-10">
            <input type="text" name="icon" id="icon" class="form-control" placeholder="Icon"
                @if ($model->exists) value="{{ $model->icon }}" @endif>
        </div>
    </div>
            ...
            <div class="form-group">
                ...
                <div class="row">
                    ...
                    @foreach ($roles as $role)
                        ...
                        <div class="col-md-6">
                            <div class="custom-control custom-checkbox">
                                <input class="custom-control-input check" type="checkbox" name="role[]"
                                    id="role{!! $role->id !!}" value="{!! $role->id !!}"
                                    {!! $check !!}>
                                <label for="role{!! $role->id !!}"
                                    class="custom-control-label">{!! $role->display_name !!}</label>
                            </div>
                        </div>
                    @endforeach
                </div>
